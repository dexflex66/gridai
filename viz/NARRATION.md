...
| **40–60s** | Toggle to **Gossip only**, hold at **step 211**. Let the staggered ripple read. | "GridAI is a priority-based coordination protocol. The Coordinator allocates each battery's dispatch slot using global fleet state to desynchronise the fleet. It converges in one to two rounds, on the inverter hardware that already exists." |
